...
export const metadata: Metadata = {
  title: "Question box",
  description: "Questions waiting to become experiments. Suggest one.",
};
...
export default async function QuestionsPage() {
  ...
  const tested = all.filter((q) => q.status === "promoted");
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 pt-10 sm:px-8">
      ...
      <p className="mt-2 max-w-xl font-serif text-[18px] text-muted">
        Things I want to understand but haven&apos;t tested yet. Some will turn into experiments. Most will stay stupid.
      </p>
      ...
      {tested.length > 0 && (
        <section aria-labelledby="tested-q" className="mt-12">
          <h2 id="tested-q" className="border-t border-ink pt-2.5 font-mono text-[12px]">
            Became experiments · {tested.length}
          </h2>
          <ul className="mt-3">
            {tested.map((q) => (
              <li key={q.id} className="border-b border-line py-3">
                ...
                {q.experimentSlug && q.experimentNumber && (
                  <Link href={`/experiments/${q.experimentSlug}`} className="ink-link font-mono text-[12px]">
                    → {experimentCode(q.experimentNumber)}
                  </Link>
                )}
              </li>
            ))}
